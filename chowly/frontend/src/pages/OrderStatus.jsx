  ...
  return (
    <div className="min-h-screen bg-cream">
      ...
      <main className="mx-auto max-w-4xl px-6 pb-24">
        ...
        {active.length > 0 && (
          <>
            {/* Mobile: stacked cards (no horizontal cut-off) */}
              ...
              {active.map((order) => {
                ...
                return (
                  <motion.button
                    ...
                    className={`w-full rounded-chowly border border-clay bg-cream-dark/50 p-4 text-left ${
                      readyToClose ? "bg-sage/10" : order.status === "DELAYED" ? "bg-red-500/10" : ""
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-semibold text-ink">#{order.id} · Table {order.visit.tableNumber}</span>
                      <StatusBadge status={order.status} />
                    </div>
                    <p className="mt-1 text-sm text-ink/70">{order.visit.customer.fullName}</p>
            ...
            <div className="mt-8 hidden overflow-hidden rounded-chowly border border-clay bg-cream-dark/50 sm:block">
                ...
                <tbody>
                  {active.map((order) => {
                    ...
                    return (
                      <motion.tr
                        key={order.id}
                        whileHover={{ backgroundColor: "rgba(255,144,48,0.06)" }}
                        onClick={() => navigate(`/waiter/order/${order.id}`)}
        ...
        {completed.length > 0 && (
          <>
            ...
            <div className="mt-4 overflow-x-auto rounded-chowly border border-clay/60 bg-cream-dark/30">
